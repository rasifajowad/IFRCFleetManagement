...
import { IconCalendar, IconClipboardList, IconCar, IconShield, IconUsersGroup, IconPlus, IconTruck, IconFileText } from '@tabler/icons-react'
...
export default async function AppSidebar({ children }: { children: React.ReactNode }) {
  ...
  const links: Array<{ label: string; href: string; icon: React.ReactNode }> = [
    ...
    { label: 'Documents', href: '/docs', icon: <IconFileText className="h-5 w-5 text-neutral-700" /> },
  ]
  if (me) {
    ...
    if (me.role === 'officer') {
      ...
      links.push({ label: 'Vehicle Info', href: '/vehicle-info', icon: <IconTruck className="h-5 w-5 text-neutral-700" /> })
    }
  } else {
    links.push({ label: 'Login', href: '/login', icon: <IconClipboardList className="h-5 w-5 text-neutral-700" /> })
  }
